...
Для контролируемого ручного добавления таблиц инвентаризации перед обновлением backend:

```bash
psql -X \
  --set=ON_ERROR_STOP=on \
  --password \
  --dbname="postgresql://USER@HOST:5432/DATABASE" \
  --file=backend/sql/2026-08-20_inventory.sql
```

Скрипт идемпотентен; после его применения `schema_ensure.py` проверяет и при необходимости
создаёт те же таблицы и индексы при старте приложения. На занятой БД скрипт не ждёт
DDL-блокировку бесконечно: через 5 секунд он завершится ошибкой и полностью откатится,
после чего его можно повторить в более спокойный момент. Пароль вводится отдельным
скрытым запросом и не попадает в историю команд.
